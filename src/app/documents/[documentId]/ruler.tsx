...
export const Ruler = () => {
    const [leftMargin, setLeftMargin] = useState(566);
            ...
            >
                ...
                <div className="absolute inset-x-0 bottom-0 h-full">
                    <div className="relative h-full w-[816px]">
                        {markers.map((marker) => {
                            const postion = (marker * 816) / 82;

                            return (
                                <div
                                    key={marker}
                                    className="absolute bottom-0"
                                    style={{ left: `${postion}px` }}
                                >
                                    {marker % 10 === 0 && (
                                        <>
                                            <div className="absolute bottom-0 w-[1px] h-2 bg-neutral-500" />
                                            <span className="absolute bottom-2 text-[10px] text-neutral-500 transform -trnaslate-x-1/2">
                                                {(marker / 10) + 1}
                                            </span>
                                        </>
                                    )}
